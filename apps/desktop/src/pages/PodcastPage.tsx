...
export default function PodcastPage() {
  ...
  return (
    <div className="podcast-page">
      ...
      {/* ── Scoped Styles ── */}
      <style>{`
        .podcast-page { padding: 0; max-width: 900px; }
        ...
        @media (max-width: 768px) { .podcast-layout { grid-template-columns: 1fr; } }
        ...
        .podcast-form-card, .podcast-progress-card { min-height: 300px; }
        ...
        .btn-option:hover:not(:disabled) { border-color: var(--accent-primary); color: var(--text-primary); }
        .btn-option.active { background: var(--accent-primary); border-color: var(--accent-primary); color: #fff; font-weight: 600; }
        .btn-option:disabled { opacity: 0.5; cursor: not-allowed; }

        .podcast-generate-btn {
          padding: 12px; font-size: 15px; font-weight: 600; border-radius: 10px;
          border: none; background: var(--accent-gradient); color: #fff;
          cursor: pointer; transition: all 0.3s; box-shadow: var(--shadow-glow);
        }
        .podcast-generate-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: var(--shadow-glow), var(--shadow-md); }
        .podcast-generate-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
        ...
        .progress-bar-container { width: 100%; height: 8px; background: var(--bg-tertiary); border-radius: 4px; overflow: hidden; }
        .progress-bar-fill {
        ...
        .progress-label { display: flex; justify-content: space-between; margin-top: 6px; font-size: 12px; color: var(--text-muted); }

        .stage-indicators {
          display: flex; justify-content: space-between;
          margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--border);
        }
        .stage-indicator { display: flex; flex-direction: column; align-items: center; gap: 6px; opacity: 0.4; transition: opacity 0.3s; }
        .stage-indicator.done { opacity: 0.7; }
        .stage-indicator.current { opacity: 1; }

        .stage-dot { width: 10px; height: 10px; border-radius: 50%; background: var(--text-muted); transition: all 0.3s; }
        .stage-indicator.done .stage-dot { background: var(--success); }
        ...
        .stage-name { font-size: 10px; color: var(--text-muted); text-align: center; max-width: 70px; }

        .audio-player-card { background: var(--bg-tertiary); padding: 16px; border-radius: 10px; border: 1px solid var(--border); }

        .podcast-empty-state {
          display: flex; flex-direction: column; align-items: center;
          justify-content: center; height: 100%; min-height: 260px; text-align: center; opacity: 0.8;
        }

        .podcast-history .jobs-list { display: flex; flex-direction: column; gap: 8px; }
        .job-item {
          display: flex; align-items: center; gap: 12px; padding: 10px 14px;
          border-radius: 8px; background: var(--bg-tertiary); border: 1px solid transparent; transition: background 0.2s;
        }
        .job-item:hover { background: var(--bg-hover); }
        .job-item.active { border-color: var(--accent-primary); }

        .job-icon { font-size: 20px; flex-shrink: 0; }
        .job-info { flex: 1; min-width: 0; }
        .job-topic { font-size: 14px; font-weight: 500; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
